...
function fn_install_wine {
    ...
    # remove
    "$UTIL" -r wine-mono wine winetricks

    # install software - wine-staging
    "$UTIL" -i wine-staging winetricks lib32-libldap lib32-gnutls lib32-mpg123 lib32-openal openal lib32-libgpg-error lib32-sqlite lib32-libpulse vulkan-drivers lib32-vulkan-icd-loader
}
...
function fn_switch_wine_prefix {
    WINE_TARGET=$(readlink -f "$HOME/.wine")
    WINE="$HOME/.wine"
    WINE_32="$HOME/wine/wine32"
    WINE_64="$HOME/wine/wine64"

    if [[ ! -L "$WINE" ]]; then
        echo "~/.wine is not a symlink, aborting"
        return 1
    fi
    rm "$WINE"
